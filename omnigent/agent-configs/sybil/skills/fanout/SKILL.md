...
2. **Per task**, create an isolated worktree on a derived branch:
   ...
   branch on the task's bd issue (the bd task backend is defined in the system
   prompt) via `$BD update <id> --notes "…" --json`.
3. **Dispatch one implementation sub-agent per task**, scoped to its worktree:
   ...
   `conversation_id` on the task's bd issue with
   `$BD update <id> --set-metadata conversation_id=<…> --json`. Emit the
   worktree + `sys_session_send` calls THIS turn, dispatch the whole
   parallel-safe set, THEN end your turn —
...
4. Each sub-agent runs autonomously and notifies you via the inbox. Collect its
   result with `sys_read_inbox` and record the outcome on the bd issue
   (`$BD update <id> --notes "<result>" --json`). If a
...
6. When cross-review passes, **merge the derived branch back**:
   ...
   resolve, then re-run cross-review on the resolution). Mark the task done
   with `$BD close <id> --json`.
