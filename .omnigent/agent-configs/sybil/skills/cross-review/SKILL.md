---
name: cross-review
description: Verify an implementer's diff with an INDEPENDENT, different-vendor sub-agent; the reviewer gets the diff, the acceptance contract, and its own isolated worktree for full code context. Blocking issues become fix-tasks; loop until clean.
---
...
3. Run the deterministic gates first — tests / lint / typecheck via
   ...
   don't involve the reviewer yet. Remove the review worktree before looping
   back (`git worktree remove .worktrees/review-<task_id>`), since re-creating
   it after fixes ensures the reviewer always sees the latest committed state.
4. Dispatch a DIFFERENT-vendor sub-agent as reviewer (Claude built it →
...
- Cross-review requires a reviewer from a DIFFERENT vendor than the implementer,
  so it needs at least two AVAILABLE workers (per sybil's roster preflight). If
